...
function mockDiseasePrediction(symptoms: string[]): {
  prediction: string;
  confidence: string;
} {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.log("/predict endpoint failed:", err);
        // Let outer catch handle mock fallback
        throw err;
      }
      ...
      if (response.ok) {
        ...
        console.log(
          `External API returned ${
            response.status
          } ${response.text()}, using mock prediction`
        );
        const mockResult = mockDiseasePrediction(symptoms);
        return NextResponse.json({
          prediction: mockResult.prediction,
